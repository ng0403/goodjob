...
<body>
	...
	<div id="title">
		<div class="caption">
		<h3 class="ui header" style="background: #fff;">■ 고객 > 키맨</h3> 
		</div>
	</div>
	...
	<div class="ui left icon input">
			<input type="text" placeholder="고객사" id="search_cust_nm" name="cont_nm" onkeydown="contactSearchEnter(event);">
			<i class="user icon"></i>
	</div>	
		<div class="ui left icon input">
			<input type="text" placeholder="이름" id="search_cont_nm" name="email" onkeydown="contactSearchEnter(event);">
			<i class="mail icon"></i>
	</div>
	...
	<div  style="height:300px">
		<table id="keymantable" class="ui sortable celled table" style="">
		<thead>
		...
		<tbody id="keyman_list_tbody" class="tbody">
		<c:forEach var="keymanList" items="${keymanList}">
 		<tr>
			<td style="text-align:center"><input type="checkbox" id="call_chek" class="call_chek" id="call_del" name="contact_del" value="${keymanList.cont_id}:${keymanList.cust_id}" onclick="callChkCancel();"></td>
			<td>${keymanList.cust_nm}</td>
			<td><a href="#" onclick="keymanDetailClick('${keymanList.cont_id}','${keymanList.cust_id}');">${keymanList.cont_nm}</a></td>
			<td style="width:20%; text-align: left; padding-left:5px;">${keymanList.cdept_nm}</td>
			<td style="width:20%; text-align: left; padding-left:5px;" >${keymanList.key_pos}</td>
 			<td style="width:20%; text-align: left; padding-left:5px;">${keymanList.key_part}</td>
			<td style="width:10%; text-align: center;">${keymanList.key_job}</td>
  		</tr>
		</c:forEach>
		</tbody>
		</table>
		...
		<div class="bottom_div">
	       <div class="functionBtn_div" id="btn_1">
	        <input type="button" class="tiny ui button" id="contactAdd_bt"  value="추가" onclick="keymanAddp();"/>
			<input type="button" class="tiny ui blue button" id="call_delete" value="삭제" onclick="keymanDelete();">
 	    </div> 
	     <div class="functionBtn_div" id="btn_2" style="display:none">
	 	    <input type="button" id="contactsave" value="저장" class="tiny ui blue button" onclick="keymanInsert();"/>
