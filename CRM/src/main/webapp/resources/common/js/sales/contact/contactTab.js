...
function callTabFunc(cont_id) {
	...
	$(document).ready(function() {
		$("#nowCall_id").val(cont_id); // 현재 sales_actvy_id 저장
			
		if($("#tab1").is(":checked")){ // 상세정보 불러오기
			if(cont_id != '') contactDetail(cont_id);
		} 
	});
}
...
/*function contactDetail(cont_id) {	
		...
		/*$.ajax({
			type : 'POST',
			url : ctx+'/contactDetail',
			data : "cont_id="+ cont_id,
			dataType : "json",
			success : function(data) {
				       
				        $("#nowCall_id").val(data.cont_id);
						$('#call_nm').val(data.cont_nm).attr("disabled", true);
						$('#cust_div_nm').val(data.cust_div_nm).attr("disabled", true);
		 
						$('#comp_nm').val(data.company_nm).attr("disabled", true);
						$('#comp_list_bt').attr("disabled",true);
						
						$('#email1').val(data.email1).attr("disabled", true);
						$('#email2').val(data.email2).attr("disabled", true);
						
						$('#cell_phone1').val(data.cell_ph1).attr("disabled", true);
						$('#cell_phone2').val(data.cell_ph2).attr("disabled", true);
						$('#cell_phone3').val(data.cell_ph3).attr("disabled", true);
