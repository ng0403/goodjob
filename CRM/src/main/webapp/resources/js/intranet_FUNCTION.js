...
function userFileDown(seq){
	var url = getContextPath()+'/home/userfiledownload';
...
function scheduleFileDown(seq){
	var url = getContextPath()+'/home/scheduledownload';
	var inputs = '<input type="hidden" name="seq" value="'+seq+'"/>';
	$('<form action="'+ url +'" method="post">'+inputs+'</form>').appendTo('body').submit().remove();
}
...
function mailFileDelete(subname){
	if(confirm('해당파일을 삭제하시겠습니까?')){
		$.ajax({
			url : getContextPath()+'/home/userFileDelete',
			data : {name : subname},
			type : 'post',
...
function scheduleFileDelete(subname){
	if(confirm('해당파일을 삭제하시겠습니까?')){
		$.ajax({
			url : getContextPath()+'/home/scheduleFileDelete',
			data : {name : subname},
			type : 'post',
			success:function(response){
				response = JSON.parse(response);
 ...
 */
function getFiles(){
	$.getJSON(getContextPath()+'/home/getFiles',{}).done(function(response){
		var result = response;
...
//			userTree.append({title : result.name+"("+result.email+")", name:result.name});
			...
			$(div).css('opacity','0.5').mouseover(function(){
				...
				if($obj.attr('clicked') == "false"){
					...
					$obj.animate({width:'385px'},800,function(){
						if(nextVo){
							getJSON(nextVo,{url: getContextPath()+'/userArticle', seq:$obj.attr('seq')});							
						}else{
							refrashRow(userArticle, {param:{page : 1}, url: getContextPath()+'/userArticle'});
...
function contentsDelete(seq){
	if(confirm('삭제 하시겠습니까?')){ 
		console.log("seq!?" + seq);
 		$.post(getContextPath()+'/scheduleDelete', {seq:seq}, function(data, textStatus) {
...
function changePage(pNo){
  	if($('#selectTab').val() == 'tab_Home'){
		refrashRow(userArticle, {param:{page : pNo}, url: getContextPath()+'/userArticle'});
	}else if($('#selectTab').val() == 'tab_Schedule'){
		refrashRow(scheduleArticle, {url: getContextPath()+'/scheduleArticle', param:{page : pNo, today : datepicker.getFormat()}});
 	}
}
...
function changePasswd(){
	...
	if(trim(oldpass) == ''){
		...
		$.ajax({
			url : getContextPath()+"/user/changePasswd",
...
function fileUpload() { 
    ...
    ( 
        { 
            url:getContextPath()+'/home/fileUpload', 
            secureuri:false, 
            type:'post',
            fileElementId:'file', 
            dataType: 'json', 
	...
	$.getJSON(param.url,param.param,function(response){
		...
		for(var i = 0 ; i < list.length ; i++){
			$(list[i].element).attr('seq',list[i].data.seq).mouseover(function(){
				...
				var url = "";
				if(userArticle.selector == table.selector){
					url = getContextPath()+"/getArticle";
				}else if(scheduleArticle.selector == table.selector){
					url = getContextPath()+"/getSchedule";
				}
				$.getJSON(url,{seq:$(this).attr('seq')},function(response){
					var article = response;
					if(userArticle.selector == table.selector){
						var html = '';
						$.getJSON(getContextPath()+'/schedule/getUserFiles',{seq:response.seq},function(response){
							var files = response;
							for(var i = 0 ; i < files.length ; i ++){
