...
public class CustcompMngController {
	...
	public @ResponseBody Map<String, Object> custcompMngAjaxList(HttpSession session, String cust_id,
			...
			@RequestParam(value = "sch_org_nm_cd", required = false) String sch_org_nm_cd,
			@RequestParam(value = "sch_org_nm_cd0", required = false) String sch_org_nm_cd0,
			@RequestParam(value = "sch_org_nm_cd1", required = false) String sch_org_nm_cd1,
		...
		if (session.getAttribute("user") == null) { // 로그인 페이지 이동
			...
			pMap.put("sch_org_nm_cd", sch_org_nm_cd);
			pMap.put("sch_org_nm_cd0", sch_org_nm_cd0);
			pMap.put("sch_org_nm_cd1", sch_org_nm_cd1);
			...
			if (cust_id == null) {
				...
				flg= 0;
				System.out.println("담당사원 리스트 확인0: " + flg);
				// 담당사원 리스트 전체 개수 조회(페이징에 사용)
				PagerVO page = ccService.custcompMngListCount(pMap);
				pMap.put("startRow", page.getStartPageNum() + "");
				pMap.put("endRow", page.getEndPageNum() + "");
				...
				List<PocVO> pocVOList = ccService.custcompMngAllList(pMap); // 담당사원 전체 리스트
				...
				flg= 1;
				System.out.println("담당사원 리스트 확인1: " + flg);
				// 담당사원 리스트 전체 개수 조회(페이징에 사용)
				PagerVO page = ccService.custcompMngListCount(pMap);
				pMap.put("startRow", page.getStartPageNum() + "");
				pMap.put("endRow", page.getEndPageNum() + "");
				...
				List<PocVO> pocVOList = ccService.custcompMngCklList(pMap); // 고객사있는 담당사원 리스트
				System.out.println("담당사원 고객사 있는 "+ cust_id+ pocVOList);
				pMap.put("ccVOList", pocVOList);
	...
	// 담당사원 수정 ajax
	@RequestMapping(value = "/ccMngModify", method = RequestMethod.POST)
	@ResponseBody
	public String ccMngModify(IuserVO iuserVo, HttpSession session) {
		System.out.println("Detail Edit Controller");

		iuserVo.setFst_reg_id(session.getAttribute("user").toString());
		iuserVo.setFin_mdfy_id(session.getAttribute("user").toString());

		System.out.println("입력된 유저ID : " + iuserVo.getFst_reg_id());
		System.out.println("수정된 유저ID : " + iuserVo.getFin_mdfy_id());

		System.out.println("담당사원 수정 iuserVo: "+ iuserVo);
		
		String result =  ccService.ccMngEdit(iuserVo);
