...
public class CustCompController {
	...
	public ModelAndView custCompList(HttpSession session,
		...
		if (session.getAttribute("user") == null) { // 로그인 페이지 이동
			...
			List<CustCompVO> SSCCodeList = ccService.selectSSC(); // 매출규모 코드
																	// 가져오기
			List<CustCompVO> IDCCodeList = ccService.selectIDC(); // 산업군 코드 가져오기
			List<CustCompVO> CCSCodeList = ccService.selectCCS(); // 기업 상태 코드
																	// 가져오기
			List<CustCompVO> CDCCodeList = ccService.selectCDC(); // 고객사 구분 코드
																	// 가져오기
	...
	public @ResponseBody Map<String, Object> custCompPList(HttpSession session,
		...
		if (session.getAttribute("user") == null) { // 로그인 페이지 이동
			...
			if (act_yn.equals("Y")) {
				...
				List<CustCompVO> ccVOList = ccService.getCCList(pMap); // 고객사
																		// 리스트
				...
				List<CustCompVO> ccVOList = ccService.getCCDelList(pMap); // 고객사
																			// 삭제된
																			// 리스트
	...
	public ModelAndView cutomerDetail(HttpSession session, @RequestParam Map<String, String> map, String cust_id,
		...
		if (cust_id == null) {
			...
			List<CustCompVO> CCSCodeList = ccService.selectCCS(); // 기업 상태 코드
																	// 가져오기
			List<CustCompVO> CDCCodeList = ccService.selectCDC(); // 고객사구분 코드
																	// 가져오기
	...
	ModelAndView custcompDelModify(@ModelAttribute CustCompVO ccVO, HttpSession session,
			@RequestParam Map<String, String> map, @RequestParam(value = "pageNum", defaultValue = "1") int pageNum,
			String cust_id) {
	...
	// 고객별 키맨//
		...
		public ModelAndView opptInsertPop(HttpSession session, String list_cust_id, String list_cust_nm,
		...
		public ModelAndView custActivePopup(HttpSession session, String list_cust_id, String list_cust_nm,
				String list_sales_oppt_id) {
...
//		ModelAndView mov = new ModelAndView("/sales/custcomp/custcompPop/custcomp_mng_pop");
	...
	public ModelAndView custCompIuserList(HttpSession session,
		...
		System.out.println("담당사원 직원검색 버튼 클릭 컨트롤러");
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("keyfield", keyfield);
		map.put("keyword", keyword);
